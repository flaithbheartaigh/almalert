...
void CLightDrvImpl::ConstructL(TBool aGameModeType) //ok
{
  ...
  iLights.SwitchLights(RDosLights::ESwitchBoth,RDosLights::ESmoothOn);
  if(iGameModeType)
  {
  ...
  switch(aType)
  {
    case EScreen:
      ...
      if(iKeysState==EOn)
      {
        if(iScreenState==EOn)
        {
          iLights.SwitchLights(RDosLights::ESwitchBoth,RDosLights::ESmoothOn); //s:on,k:on
        }
        if(iScreenState==EOff)
        {
          iLights.SwitchLights(RDosLights::ESwitchKeys,RDosLights::ESmoothOn); //s:off,k:on
        }
      }
      else
      {
        if(iScreenState==EOn)
        {
          iLights.SwitchLights(RDosLights::ESwitchScreen,RDosLights::ESmoothOn); //s:on,k:off
        }
        if(iScreenState==EOff)
        {
          iLights.SwitchLights(RDosLights::ESwitchBoth,RDosLights::ESmoothOff); //s:off,k:off
        }
      }
      break;
    case EKeys:
